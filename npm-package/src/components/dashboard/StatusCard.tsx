...
const statusConfig = {
  ...
  warning: { color: 'bg-orange-100 text-orange-800 border-orange-200', icon: '⚠', label: 'Warning' },
        ...
        {showCounts && (
          <div className="flex flex-wrap gap-2">
            {Object.entries(statusCounts).map(([status, count]: [string, number]) => (
              ...
              >
                {statusConfig[status as StatusType]?.icon} {count} {statusConfig[status as StatusType]?.label}
              </Badge>
            ))}
          </div>
        )}
